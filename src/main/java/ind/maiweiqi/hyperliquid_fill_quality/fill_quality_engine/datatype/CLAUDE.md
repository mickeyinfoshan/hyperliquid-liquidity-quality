# fill_quality_engine.datatype

Immutable data structures and enums for the fill quality engine.

## Enums

- **Side** — `BUY`, `SELL`
- **OrderEventType** — `SUBMITTED`, `FILLED`, `CANCELLED`, `MODIFIED`
- **QualityGrade** — `EXCELLENT`, `GOOD`, `FAIR`, `POOR`, `VERY_POOR` with bps thresholds

## Records

- **Tick** — Individual trade tick: `price`, `volume`, `timestamp`, `side`, `orderId`. Has validation in compact constructor.
